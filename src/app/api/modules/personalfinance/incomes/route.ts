...
export async function GET(req: NextRequest) {
  ...
  const [items, total] = await Promise.all([
    PersonalFinanceIncome.find(filter)
      .sort({ date: -1, createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean(),
    PersonalFinanceIncome.countDocuments(filter),
  ]);

  return NextResponse.json({
    items: JSON.parse(JSON.stringify(items)) as IPersonalFinanceIncome[],
    total,
    page,
    limit,
...
export async function POST(req: NextRequest) {
  ...
  const income = await PersonalFinanceIncome.create({
    workspace: auth.workspaceId,
    user: auth.userId,
    type,
    category,
