...
export async function GET(req: NextRequest) {
  ...
  const paidPlanIds = new Set(
    allPlans.filter((p) => p.monthlyPrice && p.monthlyPrice > 0).map((p) => String(p._id))
  );
  const workspacesWithPaidPlans = allWorkspaces.filter((w) => w.plan && paidPlanIds.has(String(w.plan))).length;
