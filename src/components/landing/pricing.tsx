...
export async function Pricing() {
  await dbConnect();
  const plans = await Plan.find({ isActive: true }).sort({ sortOrder: 1, name: 1 }).lean() as unknown as Array<{
    _id: string; name: string; price: string; description: string; features: string[];
    cta: string; highlighted: boolean; isActive: boolean;
  }>;
  return (
